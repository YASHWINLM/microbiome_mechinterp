...
class TranscriptomicsDataProcessor:
    ...
    def normalize(self, method: str = 'log1p', pseudocount: float = 1.0) -> None:
        ...
        """
        data = self.expression_filtered if self.expression_filtered is not None else self.expression_df
    ...
    def prepare_datasets(
        ...
        if stratify_column and self.metadata_df is not None:
            if stratify_column in self.metadata_df.columns:
                ...
                data_index = (self.expression_filtered.index if self.expression_filtered is not None 
        ...
        if test_size > 0:
            ...
            if stratify is not None:
                _, stratify_trainval = train_test_split(
                    stratify, test_size=test_size, random_state=random_state,
                    stratify=stratify
                )
            else:
    ...
    def filter_protein_coding(self, gene_type_col: str = 'gene_type') -> None:
        ...
        protein_coding = [
            col for col in self.expression_df.columns
            if 'protein_coding' in str(col).lower()
        ]
    ...
    def get_data_for_training(self) -> Tuple[np.ndarray, Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
        """
